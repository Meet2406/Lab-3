...
def process_sales_data(sales_csv_path, orders_dir_path):
    ...
    for order_id, order_df in df.groupby('ORDER ID'): 
        ...
        work_book = pd.ExcelWriter(order_file, engine='openpyxl')


        # TODO: Define format for the money columns
        money = work_book.book.add_format({'num_format': '$#,##0.00'})

        # TODO: Format each colunm
        for colnumber, value in enumerate(colnumber[1]):
            work_book.write(0, colnumber, value)
            if 'PRICE' in value or 'TOTAL' in value:
                work_book.set_column(colnumber, colnumber, None, money)
